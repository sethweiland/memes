...
#
# Priority resolution for template descriptions:
#   1. Hand-written (this dict) — version-controlled, human-curated
#   2. AI-approved (data/ai_template_descriptions.json) — AI-generated, reviewed
#   3. Empty string — no description available
#
# The review queue JSON (data/template_descriptions_review.json) is a staging
# area only and is never loaded at runtime.
...
class TemplatesCatalog:
    ...
    AI_DESCRIPTIONS_FILE = "data/ai_template_descriptions.json"
    METADATA_FILE = "data/template_metadata.json"
    ...
    def __init__(self, cache_dir: str = "data", use_expanded: bool = True):
        ...
        self.ai_descriptions_path = self.cache_dir / "ai_template_descriptions.json"
        self.metadata_path = self.cache_dir / "template_metadata.json"
    ...
    def _apply_ai_descriptions(self):
        """
        Load AI-generated descriptions and apply them to templates
        that don't already have a hand-written description.
        """
        if not self.ai_descriptions_path.exists():
            return

        try:
            with open(self.ai_descriptions_path) as f:
                ai_descs = json.load(f)
        except (json.JSONDecodeError, OSError):
            return

        self._ai_descriptions = ai_descs

        applied = 0
        for tid, entry in ai_descs.items():
            template = self.templates.get(tid)
            if template is None:
                continue
            # Only apply if template has no hand-written description
            if template.name in TEMPLATE_DESCRIPTIONS:
                continue
            if not template.description and entry.get("description"):
                template.description = entry["description"]
                applied += 1

        if applied:
            print(f"Applied {applied} AI-generated template descriptions")

    def _load_metadata(self):
        """Load template provenance metadata from sidecar file."""
    ...
    def _save_metadata(self):
        """Save template provenance metadata to sidecar file."""
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        with open(self.metadata_path, 'w') as f:
            json.dump(self.metadata, f, indent=2)

    def get_metadata(self, template_id: str) -> dict | None:
        """Get provenance metadata for a template."""
        return self.metadata.get(template_id)

    def get_description(self, template_name: str, template_id: str = "") -> str:
        """
        Get the best available description for a template.

        Priority: hand-written > AI-approved > empty string.

        Args:
            template_name: Template name (for hand-written lookup)
            template_id: Template ID (for AI description lookup)
        ...
        # 2. AI-approved
        if template_id and template_id in self._ai_descriptions:
            return self._ai_descriptions[template_id].get("description", "")
        return ""
    ...
    def get_prompt_catalog(self, limit: int = 30, randomize: bool = False) -> str:
        ...
        if randomize and len(self.templates) > limit:
            # Always include top 20 popular ones, then prefer described templates
            all_templates = list(self.templates.values())
            popular = all_templates[:20]  # Top 20 are most recognizable
            others = all_templates[20:]
            remaining_slots = limit - len(popular)

            # Split others into described and undescribed
            described = [t for t in others if t.description]
            undescribed = [t for t in others if not t.description]

            # Fill with described first, then undescribed
            picks = []
            if len(described) <= remaining_slots:
                picks.extend(described)
                fill = remaining_slots - len(described)
                if fill > 0 and undescribed:
                    picks.extend(random.sample(undescribed, min(fill, len(undescribed))))
            else:
                picks.extend(random.sample(described, remaining_slots))

            selected = popular + picks
    ...
    def search_and_add(
        ...
        Args:
            ...
            record_metadata: If True, record provenance metadata for new templates
        ...
        for meme in data['data'].get('memes', []):
            if meme['id'] not in self.templates:
                ...
                if record_metadata:
                    self.metadata[template.id] = {
                        "discovered_via": query,
                        "discovered_at": datetime.now(timezone.utc).isoformat(),
                        "source": "imgflip_search",
                        "name": template.name,
                    }
