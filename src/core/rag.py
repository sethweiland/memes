...
class DomainRAG:
    ...
    def generate_meme_ideas(
        ...
        if not context:
            print(f"No RAG context found for topic: {topic} — generating from general knowledge")
    ...
    def brainstorm_topics(
        ...
        if not context:
            # No RAG data available — brainstorm from general knowledge
            print("No RAG data available — brainstorming from general knowledge")
            prompt = f"""Suggest {num_topics} funny meme topics about {self.domain.display_name}.
Focus on well-known aspects that fans would find hilarious.
Return just the topics, one per line, no numbering."""
    ...
    def search_by_instrument(
        ...
        search_query = query or f"{instrument} {self.domain.display_name.lower()}"
        return self.search(
            search_query,
            k=k,
    ...
    def search_by_era(
        ...
        search_query = query or f"{self.domain.display_name.lower()} {start_year}s"
        return self.search(
            search_query,
            k=k,
